...
public class LDA implements Serializable {
    ...
    public void estimate(InstanceList documents, int numIterations, int showTopicsInterval,
            ...
            Randoms r, FileWriter fw) throws IOException {
        ...
        this.estimate(0, numDocs, numIterations, showTopicsInterval, outputModelInterval, outputModelFilename, r, fw);
        // 124.5 seconds
    ...
    public void printTopWords(int numWords, boolean useNewLines, FileWriter fw) throws IOException {
        ...
        for (int ti = 0; ti < numTopics; ti++) {
            ...
            if (useNewLines) {
                ...
                for (int i = 0; i < numWords; i++) {
                  ...
                  fw.write(ilist.getDataAlphabet().lookupObject(wp[i].wi).toString() + '\t');
    ...
    public void printDocumentTopics(PrintWriter pw, double threshold, int max) {
        ...
        for (int di = 0; di < topics.length; di++) {
            ...
            if (ilist.get(di).getSource() != null) {
                ...
                String Docname=ilist.get(di).getName().toString();
              
              
                pw.print(Docname.replaceAll("%20"," ").replaceAll(",",""));
    ...
    public static void runLDA( int numIterations, int numTopWords) throws IOException{
        
        String path ="output_LDA";
        new File(path).mkdirs();
          
          
        File path1 = new File(path + "\\Document_Topic.mallet");

        File path2 = new File(path + "\\Topic_Keyword.mallet.txt");
        ...
        InstanceList ilist = InstanceList.load(new File("importData.mallet"));
         numIterations = numIterations > 1 ? numIterations : 1000;
         numTopWords =  numTopWords> 2 ?  numTopWords : 20;
        System.out.println("Data loaded.");
        LDA lda = new LDA(10);
        lda.estimate(ilist, numIterations, 50, 0, null, new Randoms(5), fw);  // should be 1100
      
        
        lda.printDocumentTopics(new File(path1 + ".txt"));
    ...
    public static void main(String[] args) throws IOException {
       ...
       // File path1 = new File("D:\\Document\\thesis\\paper\\format\\springer\\mallet\\output\\Document_Topic4.mallet");

       // File path2 = new File("D:\\Document\\thesis\\paper\\format\\springer\\mallet\\output\\Topic_Keyword4.mallet.txt");
        
        //PrintWriter printWriter = new PrintWriter(path1);

    /*    FileWriter fw = new FileWriter(path2);
        InstanceList ilist = InstanceList.load(new File("importData.mallet"));
        //InstanceList ilist = InstanceList.load(new File("TESTING3.mallet"));
        int numIterations = args.length > 1 ? Integer.parseInt(args[1]) : 1000;
        int numTopWords = args.length > 2 ? Integer.parseInt(args[2]) : 20;
        System.out.println("Data loaded.");
        LDA lda = new LDA(10);
        lda.estimate(ilist, numIterations, 50, 0, null, new Randoms(), fw);  // should be 1100
        //lda.printTopWords (numTopWords, true, null);
        
        
        lda.printDocumentTopics(new File(path1 + ".txt"));

        // lda.printTopWords (numTopWords, true, fw);
        fw.close();
